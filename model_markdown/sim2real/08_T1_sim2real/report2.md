...
2. ✅ **병합·변환·검증** — 각 세션 v3→v2.1 변환 후 에피소드 재번호 병합(불완전 1개 자동 제외 후 마저 추가).
   최종 `sim_so101_blocktask_v2`: **100ep / 30,887 frames / mp4 200**, 정합(data=info=episodes=stats=100)·
   인덱스 연속성·modality(top→front/wrist→wrist)·stats count제거 **전부 검증 통과**.
   도구: `merge_blocktask_v2.py`(v2.1 다세션 병합·통계 재집계)
3. 🔄 **재학습 진행 중** — `gr00t_blocktask_v2_n16_8bit` (N1.6 8-bit, 하이퍼파라미터 동일, 데이터만 v2, 20k steps)
4. ⏳ **sim 재평가** (완주 후) — v1의 60% 대비 개선 확인 (목표 80~90%)
5. ⏳ **실기 전이 / co-training** — real gap 대비 실기 데이터 사전 수집 준비(Phase F 도구 완비)
