...
### 5.3 효과 검증 결과 (§4.7 프로토콜) — **2026-07-30**

v2 → co-train 순으로 `gr00t-srv`에 서빙하고, **같은 세션에서 연달아** 실기 성공률을 측정했다
...
**부수 발견 ② — 남은 실패 모드 = 엣지에서 블록 밀어내기.** 유일한 실패(#2)와 #10 초반 양상은 동일하다:
큐브의 **바깥쪽**을 집으려다 블록을 안쪽/바깥으로 밀어 **가동범위 밖**으로 보낸다. 일단 범위를 벗어나면
재시도도 소용없어(#2) 실패로 확정된다. §3.2에서 지목한 **엣지 커버리지·파지 정밀도** 한계가 실기에도
남아 있는 것으로, 개선하려면 **가장자리 위치의 파지-정밀 시연**을 추가 수집하는 것이 다음 방향이다.

> 정리: co-training은 **간헐성(10%→90%)** 과 **회복력** 두 축 모두를 끌어올렸고, 남은 한계는
> **엣지 파지 정밀도** 하나로 좁혀졌다.
