...
### Step 1 측정 결과 — v2(sim-only) OOD 평가 (2026-08-04)

| 조건 | SR | grasp/placed | 성격 |
|---|---|---|---|
| Baseline (학습분포 내, 고정조명) | **70%** (7/10) | 8/7 | 학습 분포 |
| DR (조명·외형 랜덤) | **80%** (8/10) | 10/8 | ~학습분포(수집을 `-DR`로 함) → **조명 강건** |
| **위치 OOD** (스폰범위 확대 0.16–0.34, 각 −0.7~1.3) | **10~30%** (반복) | **2/1**(최저) | 진짜 OOD → **붕괴** |

- **진단(§5 첫 행)**: 학습분포 70–80%로 잘 배웠으나 **위치 OOD에서 붕괴 → 학습 분포가 좁음(위치 커버리지 부족)**.
  순수 VLM 과적합 아님. 조명은 강건. → **§6 위치 전역 커버리지 재수집이 데이터로 정당화됨.**
- 위치가 어긋나면 **grasp 단계(2/10)부터** 무너짐 → 재수집 시 **위치 다양성 최우선**.
- 시행별 영상: `inf_video/01_sim_inf/{eval_baseline, eval_posood}/` (ep별 성공/실패).
- 아직 미측정 OOD: **색상·박스방향·물체** (색상은 수집 전 v2로 공짜 측정 예정).
