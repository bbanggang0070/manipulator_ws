...
### 5.1 sim에서 held-out이 성립하는 방식 (측정 프로토콜)

실기와 달리 sim은 **매 리셋마다 블록·박스 위치와 물리 파라미터가 연속 분포에서 새로 샘플링**된다
(`reset_prop_random_reach`, 마찰·질량 DR). 따라서:

- **평가 롤아웃의 초기 상태는 학습 데이터에 있던 것과 사실상 겹치지 않는다** — 연속 공간에서
  같은 값이 재현될 확률이 0에 가깝다. 즉 **평가 = 자동으로 held-out(같은 분포, 새 샘플)** 이다.
- 그래서 이 프로젝트에서 **"학습분포 내 SR" 자체가 held-out SR 역할**을 한다. 별도 분리 세트 불필요.
- 진짜 "학습셋 암기"를 보려면 **학습에 쓴 에피소드를 그대로 재생(replay)** 해 비교해야 하지만,
  GR00T는 closed-loop 정책이라 재생 자체가 성립하지 않는다 → **§5 표의 2행(암기형 과적합)은
  sim에서는 사실상 관측되지 않으며**, 판정은 아래 두 갈래로 단순화된다:

| 학습분포 SR (= held-out) | OOD SR | 해석 | 조치 |
|---|---|---|---|
| **높음(≥80%)** | 낮음(<70%) | 분포는 배웠으나 **학습 분포가 좁음** | 취약 축 특정 → 그 축을 데이터/DR에 추가 |
| **낮음(<70%)** | 낮음 | **언더핏**(태스크 난이도↑ 대비 데이터·스텝 부족) | 스텝↑ / 데이터↑ / 과한 DR 완화 |

- **N과 신뢰구간**: N=10이면 ±약 15%p, N=20이면 ±약 11%p. 축 간 비교나 v2↔v3 비교처럼 **차이가
  작을 수 있는 판단**에는 N=20 이상을 권장한다(스윕 스크립트의 두 번째 인자로 조정).
...
> [!WARNING]
> **이 v2 수치는 v3와 직접 비교하면 안 된다** (기준이 다름):
> ① 여기 "위치 OOD(0.16~0.34)"는 **v3의 학습 분포**와 같은 범위 — 같은 이름, 다른 의미
> ② baseline을 **non-DR eval**로 쟀는데 v3 기준선은 **-DR-Eval** (§5.1)
> → 공정 비교하려면 **v2를 `blocktask_ood_sweep.sh`로 재측정**해야 한다(2week_plan Week2 순서 3번).

### 데이터셋 (2026-08-05 기준)

| 데이터셋 | 구성 | 프레임 | 상태 |
|---|---|---|---|
| `sim_so101_blocktask_v3` | 100ep (v3 82 + v3_2 18) | 38,906 | 1차 학습 중 |
| `sim_so101_blocktask_v3_200` | 200ep (231ep 중 품질 선별) | 83,222 | 병합·검증 완료, 대기 |

수집 조건(전 세션 동일): 빨강 큐브 · 위치 전역(0.16~0.34, −0.7~1.25) · 박스 arc 무작위(0.28~0.34,
±66°, yaw ±π) · 물리 DR(마찰 ±30%·질량 ±50%) · 조명 DR · 카메라 고정 · 교정 시연 포함.

### 결과 기록 템플릿 (sweep 완료 시 채울 것)

`~/blocktask_ood_sweep.sh <MODEL> 20` 실행 후 `summary.txt` 값을 옮겨 적는다.
**모든 조건은 `-DR-Eval` 기준**이며 `fixed_light`만 non-DR (v2 비교용).

| 조건 | v2 재측정 | **v3 (100ep)** | **v3_200 (200ep)** | 판정 |
|---|---|---|---|---|
| `train` (in-distribution) | | | | 목표 ≥80% |
| `pos_ood` (0.12~0.38 / ±1.0~1.55) | | | | 목표 ≥70% |
| `box_ood` (arc 0.24~0.38 / ±1.45) | | | | 목표 ≥70% |
| `color_blue` | | | | 목표 ≥70% |
| `color_green` | | | | 목표 ≥70% |
| `fixed_light` (non-DR, v2비교용) | | | | 참고값 |
| **OOD 평균** (pos·box·color 4개) | | | | **목표 ≥70%** |
| 최종 loss | — | | | 언더핏 진단용 |
| 학습 epoch | — | 65.8 | 30.8 | ⚠️ 동일 스텝≠동일 epoch |

기록 시 함께 남길 것: 측정일, N(신뢰구간 — N=20이면 ±약 11%p), 조건별 소요시간(summary에 출력),
시행별 영상 경로(`outputs/sweep_<조건>/`).

### 단계 체크리스트

| 단계 | 상태 | 비고 |
|---|---|---|
| 1. v2 sim-only OOD 프로파일 | ✅ 완료 | 위치만 붕괴, 색·조명 강건 |
| 2. 씬 개조 (물리 DR·위치 전역·박스 무작위·카메라 고정) | ✅ 완료 | `blocktask_fork.patch` 반영 |
| 3. 데이터 수집 231ep → 100ep/200ep 데이터셋 | ✅ 완료 | 병합·검증 통과 |
| 4. v3(100ep) 40k 학습 | 🔄 진행 중 | 08/04 시작, ~30h |
| 5. v3 OOD sweep (N=20) | ⬜ | 학습 완료 직후 |
| 6. v2 재측정 (동일 sweep) | ⬜ | 공정 비교용 ⚠️ |
| 7. v3_200(200ep) 40k 학습 | ⬜ | 스크립트·데이터 준비 완료 |
| 8. 목표 달성 판정 (SR≥80% / OOD≥70%) | ⬜ | §2 기준 |
| 9. sim2real 전이 + co-training | ⬜ | 8 달성 후 |
