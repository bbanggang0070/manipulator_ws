...
| `real50` | `so101_blocktask_real` | 50 | 21,175 | 179M | `gr00t_blocktask_cotrain_n16_8bit` 20k | 실기 **10% → 90%** (박스 고정·era90 구도) |
| **`real60_boxdiv`** | `..._real_v2` | **60** | **41,554** | 354M | co-training v4 (예정) | **박스 다양성 + sim_v4 구도** |
...
### `real50` (50ep) — 구형
실기 SO-ARM101 수집. [report2 §5](../sim2real/08_T1_sim2real/report2.md): sim-only 10% → co-training **90%**.

> ⚠️ **두 가지 이유로 v4 co-training에서 제외한다.**
> ① **박스가 완전히 고정** — 50ep 전체에서 박스 픽셀 중심 편차가 x 7px, y 4px.
>    실기 배포에서 박스를 옮길 예정이라 박스 다양성이 0인 채로 학습된다.
> ② **카메라 구도가 다름** — 카메라를 sim v4 구도로 재정렬하며 era90 대비 세로 약 114px 이동했다.
>    섞으면 배포에서 쓰지 않을 구도를 함께 배운다.

### `real60_boxdiv` (60ep) — v4 co-training용

박스 다양성을 확보한 재수집본(`so101_blocktask_real_v2`).

| | real50 | **real60_boxdiv** |
|---|---|---|
| 박스 위치(화면 x 폭) | **7px** (고정) | **233px** |
| 박스 화면 면적 변동 | — | **2.7배** (거리 방향 분산) |
| 근접 배치 | 52% (박스 고정이라 자연 발생) | 약 40% (깊이 보정 추정) |
| 카메라 구도 | era90 | **sim_v4 정렬** |

수집 방법: [배치표](real_collect_schedule.md)를 층화 추출로 미리 뽑아 그대로 따라 놓았다.
사람이 감으로 놓으면 무의식적으로 몰리기 때문이다(real50이 그 결과다).

> 배치표는 sim의 박스 각도 범위(±66°)를 그대로 옮겼는데 **실기 카메라 화각이 sim보다 좁아**
> 각도가 큰 자세에서 박스가 프레임 가장자리에 걸렸다. 영상 확인 결과 박스가 아예 안 보이는
> 시행은 없어 그대로 사용한다. **다음 수집 시 ±45°로 좁힐 것.**
