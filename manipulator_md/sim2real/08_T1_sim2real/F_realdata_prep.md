...
- **배치(중요)**: **sim과 같은 범위 내 무작위** — 매 에피소드 큐브를 도달범위 안 **다른 위치**에 골고루
  놓는다(고정 5점 X). sim이 연속 무작위 스폰이므로 실기도 무작위여야 분포가 맞고 일반화된다.
  박스 위치는 고정. **배경·조명 일정하게.**
  > ⚠️ **5지점 그리드는 "학습"이 아니라 "평가(SR 측정)"용.** 5점으로 학습하면 그 점에 과적합돼 진짜
  > 성공률이 아니라 측정만 쉬워질 뿐. 학습=무작위 커버리지 / 평가=5점 재현측정 으로 역할을 분리한다.
...
- 권장 **20~50ep**, **sim 범위 내 무작위·균등 커버**(특정 위치 편중 금지). 부족하면 학습 시 실기 `mix_ratio`↑로 보완.
...
2. 파지 쉬운 높이로 씬 세팅 → `record_blocktask_real.sh`로 **20~50ep**(정상 ~70% + 교정 ~30%,
   **큐브는 sim 범위 내 무작위 배치**) 수집
